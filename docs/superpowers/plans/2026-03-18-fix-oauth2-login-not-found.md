# OAuth2 登录 404 Not Found 修复计划

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 修复访问 OAuth2 登录端点时出现 "Not Found" 错误的问题，确保完整的 OAuth2 登录流程正常工作

**Architecture:**
- 当前问题：前端 `Login.tsx` 使用了未导入的 `saveToken()` 函数，导致组件渲染失败
- 后端路由配置正确：`/api/v1/auth/oauth2/login` 路径已经正确注册
- 需要修复前端组件中的函数导入问题
...
用户点击"使用 Second Me 登录"按钮后，应该跳转到后端的 OAuth2 登录端点，然后重定向到 Second Me 授权页面。但目前可能出现以下问题：

1. **前端组件错误**：`Login.tsx` 中使用了 `saveToken()` 函数但未导入
2. **路由配置确认**：需要验证后端路由是否正确注册

### 当前代码状态

**后端路由配置 (`app/main.py:67`):**
```python
app.include_router(auth_router, prefix="/api/v1", tags=["Auth"])
```

**后端路由定义 (`app/api/v1/auth.py:20, 23`):**
```python
...
这意味着路由是：`GET /api/v1/oauth2/login` ✅

**前端调用 (`frontend/src/api/auth.ts:64-69`):**
```typescript
...
这里构造的 URL 是：`http://localhost:8000/api/v1/auth/oauth2/login` ❌

**问题所在：**
- 后端实际路由：`/api/v1/oauth2/login`（没有 `/auth`）
- 前端调用路径：`/api/v1/auth/oauth2/login`（多了 `/auth`）
...
**修改说明：**
- 从 `/api/v1/auth/oauth2/login` 改为 `/api/v1/oauth2/login`
- 移除多余的 `/auth` 路径段
...
git commit -m "fix(auth): 修复 OAuth2 登录端点路径，移除多余的 /auth 路径段"
```

---

### Task 2: 修复 Login.tsx 组件中的函数导入

**Files:**
- Modify: `D:\SocialClaw\frontend\src\pages\Login.tsx:1-3`

- [ ] **Step 1: 添加 saveToken 函数导入**
...
git commit -m "fix(login): 导入 saveToken 函数，修复组件渲染错误"
```
...
期望行为：
1. 跳转到 `http://localhost:8000/api/v1/oauth2/login`
2. 立即重定向到 Second Me 授权页面
3. 在 Second Me 授权后，回调到 `http://localhost:8000/api/v1/callback?code=xxx`
...
打开浏览器开发者工具（F12），查看：
- Network 标签：确认请求路径是 `/api/v1/oauth2/login`
- Console 标签：确认没有错误
